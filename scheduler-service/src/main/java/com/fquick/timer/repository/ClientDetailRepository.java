...
public interface ClientDetailRepository extends CrudRepository<ClientRegistrationDetail, Long> {
    public final static String FIND_BY_USE_CASE = "select cd from ClientRegistrationDetail cd , Client c, UseCase u where cd.client.id = c.id and u.id = cd.useCase.id and c.externalId = ?1 and u.name = ?2";
